<script>
  ...
  import {
    ...
    TEMPLATES,
    DEFAULT,
  } from "$lib/playgrounds/lego/templates.js";
  import { connMode } from "$lib/playgrounds/lego/assembly.js";
  import {
    STICK_ENDS,
    STICK_LEN,
    stickSize,
  } from "$lib/playgrounds/lego/solid.js";
  ...
  const isDescendant = (node, maybe) => {
    ...
    for (const ch of node.children ?? [])
      if (isDescendant(ch, maybe)) return true;
    return false;
  };
  function addChild(parent) {
    if (!parent) return;
    const n = {
      part: partIds.includes(sel) ? sel : (partIds[0] ?? ""),
      on: "top",
      off: [0, 0],
      rot: [0, 0, 0],
    };
  ...
  function reparent(node, newParent) {
    if (!newParent || node === model.root || isDescendant(node, newParent))
      return;
    const p = findParent(model.root, node);
    if (!p) return;
    p.children = p.children.filter((c) => c !== node);
  ...
  const parentOptions = (node) =>
    nodeList
      .filter((x) => !isDescendant(node, x.node))
      .map((x) => ({
        i: nodeIndex(x.node),
        label: `${nodeIndex(x.node)} ${x.node.part}`,
      }));
  ...
  const canHinge = (node, parent) =>
    hasHinge(parent?.part) && hasHinge(node.part);
  const hingeLabel = (o) =>
    o.end != null
  ...
  function isoModel() {
    ...
    return {
      parts: $state.snapshot(model.parts),
      baseY: 0,
      root: { part: snap.part, children: snap.children ?? [] },
    };
  }
  $effect(() => {
  ...
  {#if view === "assemble"}
    ...
    <fieldset>
      <legend
        >assembly <button
          type="button"
          onclick={() => addChild(selNode ?? model.root)}>+ child</button
        ></legend
      >
      <label
          ...
          <li>
            <label style:padding-left={`${0.3 + depth * 0.85}rem`}>
              <input
                ...
                bind:group={selNode}
              />
              <span style:background={hexOf(model.parts[node.part]?.color)}
              ></span>
      ...
      <fieldset>
        ...
        {#if parent}
          <label
            ><span>Parent</span>
            <select
              value={nodeIndex(parent)}
              onchange={(e) => reparentByIndex(node, +e.currentTarget.value)}
            >
              {#each parentOptions(node) as o}<option value={o.i}
                  >{o.label}</option
                >{/each}</select
            ></label
          >
          ...
          <label
            ...
            <select
              value={attachOf(node)}
              onchange={(e) =>
                setAttach(node, e.currentTarget.value, node, parent)}
            >
              {#each ATTACH as a}<option
                  value={a}
                  disabled={a === "hinge" && !canHinge(node, parent)}
                  >{a}</option
                >{/each}</select
            ></label
          >
          {#if attachOf(node) === "hinge"}
            {#if hingesOf(parent.part).length > 1}
              <label
                ><span>A hinge</span>
                <select
                  value={node.ah ?? hingesOf(parent.part)[0].i}
                  onchange={(e) => (node.ah = +e.currentTarget.value)}
                >
                  {#each hingesOf(parent.part) as h}<option value={h.i}
            ...
            {#if hingesOf(node.part).length > 1}
              <label
                ><span>B hinge</span>
                <select
                  value={node.bh ?? hingesOf(node.part)[0].i}
                  onchange={(e) => (node.bh = +e.currentTarget.value)}
                >
                  {#each hingesOf(node.part) as h}<option value={h.i}
                      >{hingeLabel(h.o)}</option
                    >{/each}</select
                ></label
              >
      ...
      <fieldset>
        ...
        <label
          ...
          <input
            type="text"
            value={sel}
            onchange={(e) => renamePart(sel, e.currentTarget.value)}
          /></label
        >
        <label
          ><span>Color</span>
