// Shared ink-stroke fragment core (GLSL ES 3.00). No #version / main here:
// spliced into a variant shader at its `//#include ink-core.frag.glsl` marker
// by composeShader() (src/lib/brush/shaders/index.js).
// WGSL twin: ink-core.wgsl — keep the two in sync.
//
// Mesh contract: uv01 = (perp_t, arc_t), both in 0..1; the stroke band sits in
// the central sub-rectangle [c, 1-c] × [c, 1-c] (per-axis clearances differ).
// The clearance uniforms are fed from PERP_CLEARANCE / ARC_CLEARANCE in
// src/lib/brush/ribbon.js, which builds that mesh.
...
struct StrokeField {
    float perpInPoly; // 0..1 across stroke band
    float relArc;     // 0 at tip, 1 at tail; outside [0,1] in clearance zones
    float relArcC;    // relArc clamped to 0..1
    float perpOff;    // signed perp offset from the wobbled centreline
    float d;          // round-cap SDF of the stroke band (before tip carving)
};

StrokeField strokeField(vec2 uv01) {
    float perp_t = uv01.x;
    float arc_t  = uv01.y;
...
vec4 inkStrokeColor(StrokeField f, vec2 paperUV) {
    float strandsHead = max(uStrands, 0.05) * 0.15;
    float baseFreq = 25.0 * strandsHead;
    float j1 = noise01(vec2(f.perpInPoly * baseFreq,       0.0));
    float j2 = noise01(vec2(f.perpInPoly * baseFreq * 2.3, 11.3));
    float j3 = noise01(vec2(f.perpInPoly * baseFreq * 4.7, 27.7));
    float jitter = j1 * 0.65 + j2 * 0.22 + j3 * 0.13;
    float tipPush = pow(jitter, 1.1) * 0.25;
    float d = max(f.d, -(f.relArc - tipPush));

    vec2 uvLine = vec2(f.perpOff, f.relArcC);
    float inkA = brushStrokeAlpha(uvLine, paperUV, d, uBrushColor.a);
    inkA = clamp(inkA * uOpacity, 0.0, 1.0);
    return vec4(uBrushColor.rgb, inkA);
}
