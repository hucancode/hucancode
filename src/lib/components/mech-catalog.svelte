<script>
  ...
  // ---- catalog UI surface (inlined from $lib/ui/catalog.js) ----
// CATALOG UI SURFACE — the slider rows and labels for the joint / primitive
// catalog panels shared by the mech playgrounds. The geometry they control lives
// in lib/mech (joints.js, primitives.js); the catalog model builders live in
// lib/mech/*-catalog.js.
const D = Math.PI / 180;
...
const CLEVIS = [
  ["jaw", "clevis jaw", 0.1, 0.6], ["lugT", "lug thickness", 0.05, 0.3],
  ["lugL", "lug length", 0.3, 1.2], ["lugD", "lug depth", 0.2, 1.2],
  ["pinR", "pin radius", 0.06, 0.24], ["pinOut", "pin overhang", 0, 0.2],
  ["flangeT", "flange thickness", 0.08, 0.5], ["clr", "lug clearance", 0.004, 0.08],
];
...
// one slider per DOF — the same channels a rig's bones bind to (radians; the
// prismatic slide is a distance, hence its sub-degree step)
const POSE_CTL = {
  hinge: [["swing", "swing (pin)", -90 * D, 90 * D, D]],
  pin: [["swing", "swing (pin)", -90 * D, 90 * D, D]],
  hingeTwist: [["swing", "swing (pin)", -90 * D, 90 * D, D], ["twist", "twist (disc)", -180 * D, 180 * D, D]],
  discHinge: [["spinF", "female disc spin", -180 * D, 180 * D, D], ["swing", "swing (pin)", -90 * D, 90 * D, D], ["spinM", "male disc spin", -180 * D, 180 * D, D]],
  wrist: [["bend", "bend (pin 1)", -90 * D, 90 * D, D], ["tilt", "tilt (pin 2)", -90 * D, 90 * D, D], ["twist", "twist (disc)", -180 * D, 180 * D, D]],
  pivot: [["spin", "spin", -180 * D, 180 * D, D]],
  prismatic: [["slide", "slide", 0, 0.5, 0.01]],
  ball: [["rx", "rotate x", -50 * D, 50 * D, D], ["ry", "rotate y", -180 * D, 180 * D, D], ["rz", "rotate z", -50 * D, 50 * D, D]],
};

// pose channels are radians by default; the prismatic slide is a distance
// ("raw") and passes through the UI untouched
const JOINT_POSE_UNIT = { prismatic: "raw" };
...
const PRIM_CTL = {
  ...
  const poseIsDeg = $derived(!(JOINT_POSE_UNIT[selJoint] === "raw"));
  const poseRows = $derived(poseIsDeg
    ? POSE_CTL[selJoint].map(([key, label, min, max, step]) =>
