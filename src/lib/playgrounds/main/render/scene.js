...
const DRAGON3D_LIGHT = [0.06, 0.07, 0.1];
const DRAGON3D_DARK = [0.7, 0.68, 0.59];
const _darkMQ =
...
export function makeSceneRenderer(device, canvas) {
  ...
  async function init() {
    for (const name in PROGRAMS) {
      ...
      sh[name] = device.program(def.module, def.opts);
    }
  ...
  function uploadSegs(segs) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const cx = (s.p1.x + s.p2.x) * 0.5,
        cy = (s.p1.y + s.p2.y) * 0.5;
      const hullR = Math.max(
        Math.hypot(s.p1.x - cx, s.p1.y - cy),
        Math.hypot(s.ctrl.x - cx, s.ctrl.y - cy),
      );
      segBuf[o++] = cx;
      segBuf[o++] = cy;
      segBuf[o++] = hullR;
  ...
  const bodyWidthAt = (t) =>
    _strokeW *
    (BODY_TAIL_W + (1 - BODY_TAIL_W) * smooth01(t / BODY_TAPER_SPAN));
  function drawStroke(p, points, lineWidth, opacity, vp) {
    _strokeW = lineWidth;
    const r = buildRibbon(points, lineWidth, bodyWidthAt);
    if (!r) return;
    strokePos.write(r.positions);
    strokeUV.write(r.uvs);
    strokeIdx.write(r.indices);
    p.draw(sh.stroke, {
      buffers: { aPos: strokePos, aLineUV: strokeUV },
      index: strokeIdx,
      count: r.indexCount,
  ...
  function drawDebug(p, state, aspect, vp) {
    ...
    if (state.debug.show) {
      ...
      if (p2 && p2.length) {
        // p2 is {x,y}[] -> pack xyz on z=0
        if (lineScratch.length < p2.length * 3)
          lineScratch = new Float32Array(p2.length * 3);
        for (let i = 0; i < p2.length; i++) {
          lineScratch[i * 3] = p2[i].x;
          lineScratch[i * 3 + 1] = p2[i].y;
          lineScratch[i * 3 + 2] = 0;
        }
        line(
          lineScratch.subarray(0, p2.length * 3),
          true,
          false,
          [0.9, 0.1, 0.6, 0.9],
        );
      ...
      if (state.debug.path3d && state.debug.path3d.length)
        line(state.debug.path3d, true, false, [0.0, 0.7, 1.0, 0.9]);
      if (state.debug.pool?.length)
        line(state.debug.pool, true, true, [0.2, 0.5, 1.0, 1.0]);
    }
  }
  ...
  function frame(state) {
    ...
    // clip-z convention is baked into the camera's projection already
    const vp = state.dragon3d.viewProj;

    const paper = getPaper();
    device.render({ clear: { color: paper, depth: 1 } }, (p) => {
      if (state.grid && state.grid.reveal > 0)
        p.draw(sh.grid, {
          ...
          uniforms: {
            ...
            uExt: state.grid.ext,
            uZ: state.grid.z,
            uStep: state.grid.step,
            uMinorDiv: state.grid.minorDiv,
            uOpacity: state.grid.opacity,
            uReveal: state.grid.reveal,
            uRevealMinor: state.grid.revealMinor,
            uInkColor: getInkRGB(),
          },
        });
      if (state.enso && state.enso.alpha > 0 && state.enso.sweep > 0) {
        ...
        p.draw(sh.enso, {
          ...
          uniforms: {
            uViewProj: vp,
            uOpacity: state.enso.alpha,
            uAspect: Math.max(aspect, 1),
            uZ: -0.004,
            uStationY: state.enso.stationY || 0,
            uExt: ensoExt,
            uResolution: [w, h],
            uRadius: state.enso.radius,
            uSweep: state.enso.sweep,
            uAngleStart: state.enso.angleStart,
            uLineWidth: state.enso.lineWidth,
            uInkColor: getInkRGB(),
          },
        });
      }
      if (nSeg > 0 && state.opacity.glyph > 0)
        p.draw(sh.glyph, {
          ...
          uniforms: {
            ...
            uAspect: aspect,
            uZ: -0.002,
            uStationY: state.glyph.stationY || 0,
            uExt: 1,
            uResolution: [w, h],
            uBaseRadius: state.glyph.baseRadius,
            uTime: state.glyph.playhead,
            uNSeg: nSeg,
            uInkColor: getInkRGB(),
          },
        });
      if (state.opacity.inkDragon > 0) {
        ...
        if ((d.head.alpha ?? 1) > 0)
          drawHead(
            p,
            d.head,
            (d.head.alpha ?? 1) * state.opacity.inkDragon,
            vp,
          );
      }
