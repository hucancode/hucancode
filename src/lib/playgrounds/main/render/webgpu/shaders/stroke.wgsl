// Ink ribbon stroke. WGSL port of webgl/shaders/stroke.frag.glsl (+ inline vert).
...
fn fs(in: VsOut) -> @location(0) vec4<f32> {
  ...
  let uWidthEnd = u.uWidthEnd;
  let uWidthOffset = u.uWidthOffset;
  let uWidthRange = u.uWidthRange;
  let uWidthAnchor = u.uWidthAnchor;
  let uPerpClearance = u.uPerpClearance;
  let uArcClearance = u.uArcClearance;
  let uSimple = u.uSimple;
  let brush = u.uBrushColor;
  let uWobble = u.uWobble;

  let perp_t = in.vUV01.x;
  let arc_t = in.vUV01.y;
  let invArcSpan = 1.0 / max(1.0 - 2.0 * uArcClearance, 1e-6);
  let invPerpSpan = 1.0 / max(1.0 - 2.0 * uPerpClearance, 1e-6);
  let arcInPoly = (arc_t - uArcClearance) * invArcSpan;
  let perpInPoly = (perp_t - uPerpClearance) * invPerpSpan;
  let relArc = 1.0 - arcInPoly;
  let relArcC = clamp(relArc, 0.0, 1.0);

  let halfRange = max(uWidthRange, 1e-3) * 0.5;
  let widthCurve = smoothstep(uWidthOffset - halfRange, uWidthOffset + halfRange, relArcC);
  let widthFrac = mix(1.0, clamp(uWidthEnd, 0.0, 1.0), widthCurve);
  ...
  if (uSimple == 1) {
    let aa = fwidth(perpOff) + 1e-4;
    let av = (1.0 - smoothstep(-aa, aa, d)) * brush.a * opacity;
    if (av <= 0.0) { discard; }
    return vec4(brush.rgb, av);
  }
  ...
  inkA = clamp(inkA * opacity, 0.0, 1.0);
