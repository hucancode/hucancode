...
// Pressure at arc progress s: quad value curve through (belly,k), endpoints (0,A),(1,B).
...
// Variable-radius capsule (round caps + linear taper r a->b along projection h).
...
void main() {
    ...
    for (int i = 0; i < MAX_SEG; i++) {
        ...
        vec4 H = texelFetch(uSegTex, ivec2(0, i), 0);        // center.xy, hullRadius, t0
        if (uMode == 1 && uTime <= H.w) continue;
        if (length(w - H.xy) - H.z - uBaseRadius > aa) continue;
