...
export default class Canvas3D extends React.Component {
    ...
    componentDidMount() {
        ...
        this.observer = new IntersectionObserver(
            ([entry]) => {
                this.setState(state => {return {...state, isInCamera: entry.isIntersecting};});
            }
        )
    ...
    startRecording() {
        ...
        rec.onstop = e => {
           ...
           this.setState(state => {return {...state, videoURL: URL.createObjectURL(blob)};});
        }
        rec.start();
        setTimeout(()=>{rec.stop();console.log('stopRecording');}, 20000); // stop recording in 10s
    }
    render() {
        return <Container>
